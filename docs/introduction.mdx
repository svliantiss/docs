...
<AccordionGroup>
  ...
  <Accordion title="Too many API keys" icon="gears">
    **⭕  The Problem**: New model releases, changing APIs, per-provider limits, and compliance checks make it difficult to maintain a consistent setup for your team.

    **💡 Emby’s Solution**: A unified API that lets you route to trusted providers like Azure, AWS. Bedrock, Nebius, Vertex, or DeepInfra and more.\
    \
    \* Use our routing (5%) or bring your own keys (€50/month).
  </Accordion>
  <Accordion title="Slow Inference & Compatibility Issues" icon="zap">
    ⭕ **The Problem**: AI coding tools work best when responses feel instant. In reality, performance fluctuates sometimes fast, sometimes slow depending on provider load or region. New model releases and changing APIs can also break your workflow, requiring constant adjustments.

    💡 **Emby’s Solution**: We host key models directly on fast EU servers, providing **consistent low-latency performance**. Our API stays compatible with major coding tools (Cursor, Kilo Code, Claude Code and more), even when providers change their systems.  

        Integrate Emby once, and your setup remains stable.
  </Accordion>
  <Accordion title="Vendor Lock-In & Data Privacy" icon="lock">
    ⭕ **The Problem**: Tools like Cursor & Warp constantly change their pricing, limits, and policies. What starts as “\$20/month unlimited” quietly becomes less and less more rate-limits, more timeouts, and more restrictions as your team grows.
    ...
    And if you try avoiding those costs by running your own models, you suddenly have to deal with GPUs, upgrades, scaling, networking, patches… all the things your team**does not ** want.
