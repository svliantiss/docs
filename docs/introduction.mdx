...
Emby.dev gives developer teams **easy and secure access to the best open source coding models** like Kimi, DeepSeek, GLM and Qwen without surprise token bills, proprietary lock-in, or managing GPUs, App compatibility, and Compliance.\
\
Use your existing tools (Cursor, Kilo, Warp, IDE plugins) with\
**drop-in OpenAI & Claude compatible API keys**, that can route to any of our EU hosted and compliant models.

Checkout our handy guides for integrating with your favorite tools like Kilo Code, Claude Code and more Here.
...
<CardGroup cols={2}>
  ...
  <Card title="Merchant of Record" icon="globe">
    No token billing, no surprise overages, no changing limits just a simple
...
<AccordionGroup>
  <Accordion title="Unpredictable Token Costs" icon="triangle-exclamation">
    **⭕ The Problem:** Using code editors like Cursor, Kilo code, Claude Code and Warp are expensive running up in to the €100 per day easily and costs scale unpredictably with increased team members and reliance as AI coding becomes the standard in any software developement team.\
    \
    Providers like OpenAI, Anthropic and Google overcharge you rediculously on your inference whilst training on your data, and not providing full compliance for EU dev teams.

    \*\*💡 Emby’s Solution: \*\*    Predictable subscription pricing per developer. Unlimited Emby-token usage with built-in fair usage limits based on your plan and model-based allocation right in Europe 🇪🇺 on ISO27001 and NEN7510 Certified infrastructure of **bit.nl** and fallback on **nebius.com** (Amsterdam, NL 🇳🇱)
  </Accordion>
  <Accordion title="Too many API keys" icon="gears">
    **⭕  The Problem:** To many new model releases, platforms, compliance checks and so forth. AI moves too quickly to depend on one single provider even us.

    **💡 Emby’s Solution**    : That's why we allow routing to trusted providers like AWS Bedrock, Azure, DeepInfra, Vertex, Nebius per per use for 5% routing fee or you can bring your own organization API keys for €50 per month.
  </Accordion>
  <Accordion title="Slow Inference & Compatibility Issues" icon="zap">
    ⭕ \*\*The Problem: \*\*AI coding tools work best when they feel instant. But with most AI providers, response time is slow and inconsistent sometimes fast, sometimes lagging for seconds. On top of that, every new model release, new coding api, or API standard update breaks something in your workflow. Keeping everything compatible becomes a constant headache for your team.

    💡 \*\*Emby’s Solution: \*\*We run the most important models directly on fast EU servers, so your team gets **stable, low-latency responses every time**. Our API is fully compatible with the major coding tools (checkout our guides for Kilocode, Claude Code, and more here) and stays consistent,even when providers change or update their systems. You integrate once Emby dev once, and everything keeps working.
  </Accordion>
  <Accordion title="Vendor Lock-In & Data Privacy" icon="lock">
    ⭕ **The Problem:** Tools like Cursor, Kilo and Warp constantly change their pricing, limits, and policies. What starts as “\$20/month unlimited” quietly becomes less and less — more rate-limits, more timeouts, and more restrictions as your team grows.

    Most of these editors route your requests through OpenAI, Anthropic or OpenRouter, which means:

    - Unpredictable token costs
    - No real EU compliance
    - Your code and prompts leaving the EU
    - Premium model pricing that becomes brutally expensive at scale

    And if you try avoiding those costs by running your own models, you suddenly have to deal with GPUs, upgrades, scaling, networking, patches… all the things developers and you \*\*_don’t_ \*\*want to deal with.

    💡 **Emby’s Solution:** Predictable model access with a simple subscription per developer. Fully EU-hosted. No price shifts. No lock-in. No surprise token bills. And no managing GPUs or complicated infrastructure  we handle the heavy lifting while your team just uses the models they want.
...
<Steps>
  <Step title="Create Your Account">
    ...
    <img
      className="block dark:hidden"
      src="/assets/create-org.light.png"
    />

    <img
      className="hidden dark:block"
      src="/assets/create-org.dark.png"
    />
  </Step>
...
<Info>
  **Additional fees may apply**: Some transactions may incur additional fees (international cards, subscriptions). Payout fees are charged by payment providers. See our [detailed fees page](/merchant-of-record/fees) for complete information.
</Info>
